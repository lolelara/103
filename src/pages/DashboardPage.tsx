...
const DashboardPage: React.FC = () => {
  ...
  switch (user.role) {
    ...
    case UserRole.COACH:
      roleSpecificLinks = [
        { path: 'coach', label: 'لوحة المدرب', icon: 'fas fa-chalkboard-teacher' },
        { path: 'coach/my-trainees', label: 'متدربيني', icon: 'fas fa-users' },
        { path: 'coach/videos', label: 'فيديوهات التمارين', icon: 'fas fa-video' },
        { path: 'coach/nutrition', label: 'خطط التغذية', icon: 'fas fa-apple-alt' },
        { path: 'coach/manage-schedules', label: 'إدارة الجداول', icon: 'fas fa-calendar-check' },
      ];
      break;
    case UserRole.NUTRITIONIST:
      roleSpecificLinks = [
        { path: 'nutritionist', label: 'لوحة طبيب التغذية', icon: 'fas fa-user-md' },
        { path: 'nutritionist/trainees', label: 'متابعة المتدربين', icon: 'fas fa-users' },
        { path: 'nutritionist/plans', label: 'خطط غذائية', icon: 'fas fa-apple-alt' },
        { path: 'nutritionist/notifications', label: 'إشعارات غذائية', icon: 'fas fa-bell' },
      ];
      break;
    case UserRole.TRAINEE:
      roleSpecificLinks = [
        ...
        { path: 'trainee/rate-trainer', label: 'تقييم المدرب', icon: 'fas fa-user-check' },
      ];
      break;
  }
        ...
        <Routes>
          ...
          {user.role === UserRole.COACH && (
            <>
              ...
              <Route path="coach/my-trainees" element={<CoachDashboard />} />
              <Route path="coach/videos" element={<CoachDashboard />} />
              <Route path="coach/nutrition" element={<CoachDashboard />} />
              <Route path="coach/manage-schedules" element={<CoachDashboard />} />
            </>
          )}
          {user.role === UserRole.NUTRITIONIST && (
            <>
              <Route index element={<Navigate to="nutritionist" />} />
              <Route path="nutritionist" element={<NutritionistDashboard />} />
              <Route path="nutritionist/trainees" element={<NutritionistDashboard />} />
              <Route path="nutritionist/plans" element={<NutritionistDashboard />} />
              <Route path="nutritionist/notifications" element={<NutritionistDashboard />} />
            </>
          )}
