// جميع الأنواع في هذا الملف تدعم التشغيل والتسجيل على localhost وبيئة الإنتاج (production)
// لا توجد أي متغيرات أو أنواع تعتمد على بيئة التشغيل
// يمكن استخدام هذه الأنواع في أي بيئة بدون تعديل
// All types in this file are designed to fully support local development (localhost) and production environment for registration and login.
// -------------------------------------------------------------
...
export interface User {
  ...
  coachId?: string | null; // UUID from DB, can be null
  traineeIds?: string[]; // For coaches, IDs of their assigned trainees
...
export interface TrainingVideo {
  ...
  title: string;
  youtubeUrl: string;
  uploadedBy: string; // User ID (coach or admin)
...
export interface Rating {
  id:string; // UUID from DB
  ratedItemId: string; // Plan ID أو Coach ID
  ratedItemType: 'plan' | 'coach';
